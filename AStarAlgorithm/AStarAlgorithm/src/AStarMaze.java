...
public class AStarMaze extends JFrame implements Runnable, MouseListener, MouseMotionListener, KeyListener {
    ...
    public void mousePressed(MouseEvent e) {
    	if (!isGameRunning) {
    		...
    		int x = e.getX();
    		int y = e.getY();
    		if (x>=15 && x<=85 && y>=90 && y<=120) {
    			isGameRunning=true;
    			badguy.reCalcPath(map, player.x, player.y);	
    			return;
    		}
    		// or the 'load' button?
    		if (x>=315 && x<=385 && y>=90 && y<=120) {
    			loadMaze();
    			return;
    		}
    		// or the 'save' button?
    		if (x>=415 && x<=485 && y>=90 && y<=120) {
    			saveMaze();
    			return;
    		}
    	}
	...
	public void paint(Graphics g) {
        ...
        if (!isGameRunning) {
        	...
        	g.setColor(Color.GREEN);
        	g.fillRect(15, 90, 70, 30);
        	g.fillRect(315, 90, 70, 30);
        	g.fillRect(415, 90, 70, 30);
        	g.setFont(new Font("Times", Font.PLAIN, 24));
        	g.setColor(Color.BLACK);
        	g.drawString("Start", 22, 112);
        	g.drawString("Load", 322, 112);
        	g.drawString("Save", 422, 112);
        }
		// flip the buffers
		strategy.show();
	}
